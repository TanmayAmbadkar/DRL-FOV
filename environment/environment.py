...
class Environment:
        ...
        try:
            return np.sum(
                current_observation[
                    ...
                    + self.frame_height // 2,
                    self.current_x
                    - self.frame_width // 2 : self.current_x
                    + self.frame_width // 2,
                ]
            ) - (
                np.sum(
                    last_observation[
                        :,
                        last_y
                        - self.frame_height // 2 : last_y
                        + self.frame_height // 2,
                        last_x - self.frame_width // 2 : last_x + self.frame_width // 2,
                    ]
                )
            )
        except:
    ...
    def step(self, action):
        ...
        last_observation = self.saliency_frames[
            self.current_frame - self.n_frames : self.current_frame, :
        ]
        current_observation = self.saliency_frames[
            self.current_frame : self.current_frame + self.n_frames, :
        ]
        self.current_frame += self.n_frames
        done = self.current_frame - self.start_frame == self.video_frames
        reward = self.reward(
            self.sigmoid(last_observation),
            self.sigmoid(current_observation),
            last_x,
            last_y,
        )

        if not done:
            next_observation = self.saliency_frames[
                self.current_frame : self.current_frame + self.n_frames, :, 240:720
            ]
    ...
    def reset(self):
        ...
        self.start_frame = np.random.randint(
            self.total_frames - self.video_frames - self.n_frames - 1
        )
        self.current_frame = self.start_frame + self.n_frames
        self.current_x = 480
        self.current_y = 120
        return self.sigmoid(
            self.saliency_frames[
                self.start_frame : self.start_frame + self.n_frames, :, 240:720
            ]
        )

    def render(self):

        current_observation = self.colored_frames[
            self.current_frame - self.n_frames : self.current_frame, :
        ]
